...
void callback(String topic, byte* message, unsigned int length) {
  ...
  if(topic=="g04"){
      Serial.print("Opening  main door ");
      if(messageTemp == "true"){
        ...
        Serial.print("Closing main door");
...
client.publish("Lock/g04",lock1string);
client.publish("Unlock/g04",unlock1string);
//client.publish("Lock/cabin",lock2string);
//client.publish("Unlock/cabin",unlock2string);
client.publish("g04/in",intime1);
client.publish("g04/out",outtime1);
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("ground")) {
      Serial.println("connected");  
      digitalWrite(door3,HIGH);
      client.subscribe("g04");
